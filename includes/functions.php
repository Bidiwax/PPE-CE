...
$reponse = $bdd -> query("SELECT * FROM news ORDER BY ID DESC"); // Requête SQL triée par ID descendant

$membres = $bdd -> query ("SELECT * FROM members ORDER BY ID"); // Requête SQL triée par ID descendant
...
function voyages_disponibles() {
	return $connexion = $bdd->query("SELECT COUNT(*) FROM deals WHERE Categorie=Voyages")->fetchColumn();
}

function sorties_disponibles() {
	return $connexion = $bdd->query("SELECT COUNT(*) FROM deals")->fetchColumn();
}
 
function cinema_disponibles() {
	return $connexion = $bdd->query("SELECT COUNT(*) FROM deals WHERE Categorie=Tickets")->fetchColumn();
}
 
function users_actuels() {
	return $connexion = $bdd->query("SELECT COUNT(*) FROM members")->fetchColumn();
}
...
// Cette fonction va comparer l’adresse email et le mot de passe avec ceux présents dans la base de données. Si ce sont les mêmes, true sera renvoyé
...
// Vérification de BruteForce, 5 essais max
